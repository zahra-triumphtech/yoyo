#Create subnets for Ingress gwlbe subnets
resource "aws_subnet" "gwlbe_subnet" {
  for_each          = { for data_subnet in var.gwlbe_subnet_configs : data_subnet.az => data_subnet }
...
resource "aws_route_table_association" "gwlbe_subnet_route_table_association" {
  for_each       = { for data_subnet in var.gwlbe_subnet_configs : data_subnet.az => data_subnet }
  subnet_id      = aws_subnet.gwlbe_subnet[each.key].id
  route_table_id = aws_route_table.gwlbe_route_table[each.key].id
}