...
resource "aws_subnet" "inspection_mgmt_subnet" {
  for_each          = { for inspection_mgmt_subnet in var.mgmt_subnet_configs : inspection_mgmt_subnet.az => inspection_mgmt_subnet }
...
resource "aws_route_table" "inspection_mgmt_route_table" {
  vpc_id = aws_vpc.inspection_vpc.id
  tags   = merge(var.tags, { "Name" : "${var.tags["ClientName"]}-${var.type}-inspection_mgmt-Subnet-RT" })
}
